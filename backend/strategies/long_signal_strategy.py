...
class LongSignalStrategy(BaseStrategy):
    """
    Parameters mirror growth_signal_bot_v4.pine inputs.
    Override any param at instantiation:
        strategy = LongSignalStrategy(entry_mode="Strong Buy Only", sl_pct=6, tp_pct=18)

    For 1H charts use the factory (sets all 1H-appropriate defaults):
        strategy = LongSignalStrategy.for_1h(entry_mode="Buy Only")
    """

    name        = "LongSignalStrategy v4.3"
    ...
    @classmethod
    def for_1h(cls, **overrides) -> "LongSignalStrategy":
        """
        Factory: LongSignalStrategy pre-configured for 1H charts (v1.4 logic).
        ...
        Override any param at call time:
            bot = LongSignalStrategy.for_1h(entry_mode="All Signals", sl_pct=1.5)
        """
        defaults: dict = dict(
            macd_fast           = 8,
            macd_slow           = 21,
        ...
        bot      = cls(**defaults)
        bot.name = "LongSignalStrategy 1H v1.4"
        return bot
